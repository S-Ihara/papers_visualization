...
class ICCV_papers_collecter:
    """Collect papers info from ICCV
    Note:
    ...
    def __init__(self,year=2023 ,data_path=Path("./papers_info")):
        ...
        """
        self.conference_url = f"https://openaccess.thecvf.com/ICCV{year}?day=all"
        ...
        if 2012 < year <= 2020:
            self.conference_url = f"https://openaccess.thecvf.com/ICCV{year}"
    ...
    def save_pickles(self,save_path,mask_length=200):
        ...
        mask = np.array([len(a) >= 200 for a in iccv.abstract])
        iccv = iccv[mask].reset_index(drop=True)
        print(f'Removing {np.sum(~mask)} submissions with abstract length below 200 characters.')

        if isinstance(save_path, str):
            save_path = Path(save_path)
        iccv.to_pickle(save_path / f'iccv_{self.year}.pickle')
        print(f"iccv_{self.year}.pickle is saved at {save_path}")